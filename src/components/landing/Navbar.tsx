...
type NavLink = {
  label: string;
  href: string;
  /** Extra visual weight (e.g. key conversion paths) */
  emphasis?: boolean;
};

/** Grouped for clearer scannability; dividers only on desktop pill */
const navLinkGroups: NavLink[][] = [
  [
    { label: "Home", href: "#" },
    { label: "Treatments", href: "/treatments", emphasis: true },
  ...
  [
    ...
    { label: "Contact", href: "#booking", emphasis: true },
  ],
];
...
const Navbar = () => {
  ...
  /** Non-home: chrome bar. Home + scrolled: same transparent + frosted pill as hero, text turns dark only */
                ...
                {group.map((link) => (
                  <li key={link.href} className="shrink-0 list-none">
                    <button
                      ...
                      className={cn(
                        "rounded-md px-1 py-0.5 text-left text-sm transition-colors sm:px-1.5 sm:py-1 sm:text-base",
                        isOtherPage &&
                        ...
                        isHome &&
                          !homeTextDark &&
                          "font-medium text-white hover:bg-white/15 hover:text-white",
                        isHome &&
                          !homeTextDark &&
                          link.emphasis &&
                          "font-semibold !text-white drop-shadow-sm",
                        homeTextDark &&
                          "font-medium !text-neutral-900 hover:!bg-black/10 hover:!text-black",
                        homeTextDark && link.emphasis && "font-semibold !text-black"
                      )}
                    >
                      {link.label}
                    </button>
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
              <Button
                ...
                className={cn(
                  "h-10 w-10 md:hidden",
                  isOtherPage && "text-chrome-foreground hover:bg-foreground/10",
                  isHome && !homeTextDark && "text-white hover:bg-white/10",
                  homeTextDark && "text-neutral-900 hover:bg-black/10"
                )}
              >
            ...
            <SheetContent side="right" className="w-72 bg-background">
              ...
              <nav className="flex flex-col gap-0" aria-label="Main navigation (mobile)">
                {navLinkGroups.map((group, gIdx) => (
                  <div
                    key={gIdx}
                    className={cn("pb-2 pt-1", gIdx > 0 && "mt-1 border-t border-border/60 pt-3")}
                  >
                      ...
                      {group.map((link) => (
                        <li key={link.href}>
                          <button
                            onClick={() => handleNav(link.href)}
                            className={cn(
                              "w-full rounded-md py-2.5 text-left text-base transition-colors",
                              link.emphasis
