...
function MemberCard({ name, role, image }: { name: string; role: string; image?: string }) {
  ...
  return (
    <div className="text-center">
      <div className="w-20 h-20 sm:w-24 sm:h-24 mx-auto rounded-xl bg-white/10 border border-white/20 mb-3 flex items-center justify-center overflow-hidden shrink-0">
        <img src={src} alt={name} className="w-full h-full object-cover" />
      </div>
      <h3 className="font-rounded text-base font-semibold text-white">{name}</h3>
...
const TeamSection = () => (
  <section id="team" className="py-12 lg:py-16 bg-dark-emerald-gradient">
    <div className="container mx-auto px-4 lg:px-8">
      ...
      >
        <p className="text-sm font-sans font-semibold tracking-widest uppercase text-white/90 mb-2">
          Our People
        </p>
